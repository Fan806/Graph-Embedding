...
def dfs(u, tree, handlers, params, res_radius, res_coordinates):
    ...
    print "res_coordinates[u]:",res_coordinates[u]
    print "res_radius[u]:",res_radius[u]

    # if (len(node_in_tree) <= 2):
    #     print("always less than or equal to 2")
    ...
    #     # print("node_in_tree:",node_in_tree)
    ...
    #         print(rc_b)
    #     # print("????????????",res_coordinates[node_in_tree[0]]-res_coordinates[node_in_tree[1]])
    # else:
    # neural embedding
    ...
    for i in xrange(len(r)):
        for j in xrange(len(r)):
            if sim_mat[i][j] > sys.float_info.epsilon:
                # print("np.linalg.norm:",np.linalg.norm(res_coordinates[node_in_tree[i]] - res_coordinates[node_in_tree[j]]))
...
def metric(params):
    ...
    print(type(origin_coordinates))
    # res = getattr(Metric, metric["metric_func"])(origin_coordinates, metric)
    # ret.append((metric["metric_func"], res))
    print(len(origin_coordinates[0]))
    kmeans = KMeans(n_clusters=10).fit_predict(origin_coordinates)
...
def main():
    ...
    parser.add_argument('--conf', type = str, default = "real2")
    # parser.add_argument('--conf', type = str, default = "hamilton_real2")
